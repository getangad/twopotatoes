...
//join room
function joinRoom() {
  console.log("started to join room " + document.querySelector(
      'input[name="player-team"]:checked').value + " "
...
socket.on(ClientHandledEvents.ROOM_CREATED, function (data) {
  ...
  playerInfo.team = data.team;
  playerInfo.name = data.name;
  document.getElementById('roomID').innerHTML += '<p><strong> PIN : '
      + data.roomPin + '</strong></p>';
  document.getElementById(
      'playerCount').innerHTML = '<p><strong> number of players : '
      + data.numberOfPlayers + '</strong></p>';
  document.getElementById('playerDivMsg').innerHTML = '' +
